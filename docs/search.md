...
so a recent incidental mention cannot displace credible matches. In the mixed `All` view, freshness reorders Blog results
only within the positions Pagefind assigned to Blog; it does not create extra Blog positions above Website results. The
Blog filter still gets the full relevance/freshness blend and exposes a `Newest` option that uses Pagefind's native
`published: desc` sort without fusion. The pure ranking policy lives in `src/data/search-ranking.ts`; tune it against
representative queries and its deterministic unit tests, not a single production query.

Website records also carry a normalized exact-title filter generated by the shared layouts. The dialog performs a small
filtered lookup alongside the main search and places an exact Website-title match before ordinary candidates. This makes
navigational queries such as `xcp-ng` resolve to the canonical Website page without requiring per-page promotion rules,
while retaining matching Blog posts immediately below it.
