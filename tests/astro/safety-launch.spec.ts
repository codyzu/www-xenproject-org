...
const safetyDescription = 'Open source safety engineering through the Xen Safety Committee for safety-critical and mixed-criticality systems, with certification-oriented processes, evidence, tooling, and long-term platform control.';
...
test.describe('Safety launch integration', () => {
  test('presents the homepage launch section in the intended narrative position', async ({page}) => {
    ...
    await expect(section).toContainText('Xen Safety Committee and Premier Plus membership');
  ...
  test('integrates concise Safety Committee context into Governance', async ({page}) => {
    ...
    await expect(governanceContent).toContainText('Premier Plus members appoint voting representatives');
